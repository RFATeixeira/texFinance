...
export default function CartoesList({ onAdd, showAll, setShowAll }: Props) {
  ...
  const unsubscribeAuth = onAuthStateChanged(auth, (user) => {
    ...
    const q = query(collection(db, "users", user.uid, "cartoesCredito")); // ou "cartoesCredito"
        ...
        <>
          {cartoes
            ...
            .map((cartao) => {
              ...
              return (
                <div key={cartao.id} className="bg-gray-50 p-4 rounded-xl text-gray-800">
                  ...
                  <div className="mt-2 text-xs text-gray-500 space-y-1">
                    <p>
                      Fecha <span className="text-sm font-semibold">{cartao.diaFechamento}/{fechamentoDate.format("MM")}</span> ·
                      Vence <span className="text-sm font-semibold">{cartao.diaVencimento}/{vencimentoDate.format("MM")}</span>
                    </p>
                    <p>
